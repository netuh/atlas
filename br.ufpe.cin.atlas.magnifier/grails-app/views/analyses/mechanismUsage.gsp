...
<script type="text/javascript">
	      ...
	      function drawChart() {
			...
			<g:each in="${relevant}" var="mech">
				,['${mech.key.getMechanismId()}',     ${mech.value}]
			</g:each>
	        ]);
...
<body>
	...
	<g:form controller="analyses">
		...
		<g:actionSubmit action="mechanismUsage"
	...
	<dl>
		<g:each in="${relevant}" var="study">
			<dt>
				<g:link action="show" controller="usedMechanism" id="${study.key.getId()}">${study.key.getMechanismId()}</g:link>. Total: ${study.value} 
			</dt>
		</g:each>
	</dl>
</body>